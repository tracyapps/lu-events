...
class ACFE_Updates{
    ...
    function html(){
        ...
        $icon = '';
        if(acfe_is_acf('6.0')){
            $icon = '<i class="acf-icon-key"></i>';
        }
        
        ?>
        <div class="acf-box" id="acfe-license-information">
            <div class="title">
                <h3><?php echo $icon; ?> <?php _e('ACF Extended: License Information', 'acf'); ?></h3>
            </div>
            <div class="inner">
                ...
                <form action="" method="post">
                    ...
                    <?php if(acfe_is_acf('6.2.2')): ?>

                        <label for="acfe_pro_licence"><?php _e('License Key', 'acf'); ?></label>
                        
                        <?php
                        $btn_class = $active ? 'acf-btn-tertiary' : '';
                        ...
                        <input type="submit" value="<?php echo esc_attr($button); ?>" class="acf-btn <?php echo $btn_class; ?>">
                        <?php if(!$active): ?>
                            <a href="https://www.acf-extended.com/account" target="_blank" class="acf-manage-license-btn">
                                <?php echo __('Manage License', 'acf'); ?><i class="acf-icon acf-icon-arrow-up-right"></i>
                            </a>
                        <?php endif; ?>
                    
                    <?php elseif(acfe_is_acf('6.0')): ?>
            ...
            <?php if(acfe_is_acf('6.2.2')): ?>
                ...
                <?php
                $no_license_text = __('Don\'t have an ACF PRO license? %s', 'acf');
                $no_license_text = str_replace('%s', '', $no_license_text);
                
                if(stripos($no_license_text, 'ACF PRO') !== false){
                    $no_license_text = str_ireplace('ACF PRO', 'ACF Extended Pro', $no_license_text);
                }else{
                    $no_license_text = '';
                }
                
                $label_class = 'inactive';
                $label_text = __('Inactive', 'acf');
                
                if($active){
                    $label_class = 'active';
                    $label_text = __('Active', 'acf');
                }
                ?>
                ...
                <div class="acf-license-status-wrap">
                    ...
                    <table class="acf-license-status-table">
                        <tbody>
                            <tr>
                                <th><?php echo __('License Status', 'acf'); ?></th>
                                <td><span class="acf-license-status <?php echo $label_class; ?>"><?php echo $label_text; ?></span></td>
                            </tr>
                        </tbody>
                    </table>
                    <?php if(!$active): ?>
                        <div class="acf-no-license-view-pricing">
                            <span>
                                <?php echo $no_license_text; ?> <a href="https://www.acf-extended.com/pro" target="_blank"><?php echo __('View pricing & purchase', 'acf'); ?> <i class="acf-icon acf-icon-arrow-up-right"></i></a>
                            </span>
                        </div>
                    <?php endif; ?>
                </div>
        ...
        $icon = '';
        if(acfe_is_acf('6.0')){
            $icon = '<i class="acfe-icon-info"></i>';
        }
        ?>
        <div class="acf-box" id="acfe-update-information">
            <div class="title">
                <h3><?php echo $icon; ?><?php _e('ACF Extended: Update Information', 'acf'); ?></h3>
            </div>
            <div class="inner">
                <table class="form-table">
                    <tbody>
                    ...
                    <?php if(acfe_is_acf('6.0')): ?>
                        <tr class="acfe-update-button-row">
                            ...
                            <td colspan="2">
                                ...
                                <?php else: ?>
                                    <a class="button" href="<?php echo esc_attr(add_query_arg('acfe-pro-check', 1)); ?>"><?php _e('Check For Updates', 'acf'); ?></a>
                                <?php endif; ?>
                            </td>
