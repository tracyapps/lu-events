...
class ACFE_World_Query{
    ...
    function filter(){
        ...
        // vars
        $args = acfe_as_array($this->args);
        $data = acfe_as_array($this->data);
        ...
        foreach($_args as $rule){
            ...
            // filter
            if(acfe_get($split, 1) !== 'in') continue;
        ...
        foreach(array_keys($data) as $key){
            ...
            foreach($rules as $r_key => $rule){
                ...
                $args[$rule] = acfe_as_array($args[$rule]);                  // array('us', 'fr', 'de')
    ...
    function order(){
        ...
        // orderby: name__in
            ...
            $key = $columns[0];                         // name
            $array = acfe_as_array($args[$orderby]);    // array('fr', 'us', 'de')
