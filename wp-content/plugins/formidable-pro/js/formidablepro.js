function frmProFormJS() {
	...
	frmDatepickerPro.getDefaultConfigs = function( config, input ) {
		...
		const flatpickrOptions = {
			...
			onChange: ( selectedDates, dateStr, instance ) => {
				if ( instance.config.inline ) {
					const altInput = instance.config.altInputElement || instance.altInput;
					if ( altInput ) {
						altInput.value = dateStr;
	...
	frmDatepickerPro.setDefaultRangeValue = function( instanceElement ) {
		...
		if ( instanceElement._flatpickr.config.inline ) {
			...
			if ( ! input ) {
				const container = instanceElement.closest( '.frm_repeat_sec, .frm_repeat_inline, .frm_repeat_grid' );
				if ( container ) {
					input = container.querySelector( `input[name^="item_meta[${ fieldId }]["]` );
				}
			}
			if ( ! input ) {
				return;
	...
	function loadDropzone( i, repeatRow ) {
		...
		if ( repeatRow !== undefined && selector.includes('-0_dropzone') ) {
			...
			label = label.replace( /-0$/, '-' + repeatRow );
	...
	*/
	...
	function copyRootlines( listWrappers ) {
		var i, listWrapper, form, clone, rootlinesBackup;
		rootlinesBackup = {};
		for ( i = 0; i < listWrappers.length; i++ ) {
			listWrapper = listWrappers[ i ];
			form = listWrapper.closest( 'form' );
			clone = listWrapper.cloneNode( true );

			// Don't bake a transient disabled state (eg. mid dependent-field load) into the
			// snapshot used to rebuild the rootline on resize, or the "Show More" button gets
			// stuck disabled forever after a resize lands during that window.
			if ( ! form.classList.contains( 'frm_loading_form' ) ) {
				clone.querySelectorAll( 'input[type="button"]' ).forEach( button => button.disabled = false );
			}

			rootlinesBackup[ form.getAttribute( 'id' ) ] = clone;
		}
	...
	function showMoreStepsButtonEvents() {
		...
		// Some mobile browsers (eg. iOS Safari) only fire visualViewport's resize event, not
		// window's, when the address bar collapses/expands without changing the layout viewport.
