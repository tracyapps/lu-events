...
class acfe_payment extends acf_field{
    ...
    function paypal_create($args, $field, $post_id, $acf){
        ...
        acfe_setup_meta($acf, 'acfe/paypal_create', true);
            ...
            // defaults
            $amount = acfe_get($args, 'amount', 0);
            $currency = acfe_get($args, 'currency', 'USD');
            $description = acfe_get($args, 'description');
    ...
    function payment_confirm($response, $field, $post_id, $acf){
        ...
        // vars
        $id = acfe_get($response, 'id');
        $gateway = acfe_get($response, 'gateway');
        $object = acfe_get($response, 'object');
        ...
        if($gateway === 'stripe'){
            ...
            // vars
            $amount = acfe_get($object, 'amount', 0);
            $currency = strtoupper(acfe_get($object, 'currency', 'USD'));
            ...
            // vars
            $amount = acfe_get($object, 'PAYMENTINFO_0_AMT', 0);
            $currency = strtoupper(acfe_get($object, 'PAYMENTINFO_0_CURRENCYCODE', 'USD'));
    ...
    function validate_payment_object($value){
        ...
        // items
        $value['items'] = acfe_as_array($value['items']);
    ...
    function get_gateway_api($field, $gateway, $name){
        ...
        // return
        return acfe_get($field, "{$gateway}_{$prefix}{$name}", '');
