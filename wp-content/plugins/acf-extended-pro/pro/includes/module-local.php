...
class acfe_module_local{
    ...
    function updated_item($item, $module){
        ...
        if($item['ID']){
            ...
            // get item from db
            $db_item = $module->get_item($item['ID'], 'db');
            
            // delete old file if name changed
            if($db_item['name'] && $db_item['name'] !== $item['name'] && !acf_is_filter_enabled('acfe/module/update_unique_name')){
                
                if(acfe_has_php_sync($item)){
                    $this->delete_php_file($db_item, $module, true);
                }
                
                if(acfe_has_json_sync($item)){
                    $this->delete_json_file($db_item, $module, true);
                }
    ...
    function add_files($files){
        ...
        foreach(acfe_as_array($files) as $file){
            $store->append($file);
        }
    ...
    function get_items($module){
        ...
        foreach($local_items as $local_item){
            ...
            // assign db item data
            $db_item = $module->get_item($name, 'db');
        ...
        foreach($local_items as $local_item){
            ...
            // extract vars
            $name     = acfe_get($local_item, 'name');
            $local    = acfe_get($local_item, 'local');
            $file     = acfe_get($local_item, 'local_file');
            $modified = acfe_get($local_item, 'modified');
            $private  = acfe_get($local_item, 'private');
    ...
    function get_item_status($item, $type, $module){
        ...
        // no local sync files found
            ...
            $item_has_sync = in_array($type, (array) acfe_get($item, 'acfe_autosync', array()));
