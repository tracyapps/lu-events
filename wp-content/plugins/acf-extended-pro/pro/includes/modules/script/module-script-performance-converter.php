...
class acfe_script_performance_converter extends acfe_script{
    ...
    function process_post_types(){
        ...
        // Get task
        $post_types = acfe_get($this->data['tasks'], 'post_types', array());
    ...
    function process_taxonomies(){
        ...
        // Get task
        $taxonomies = acfe_get($this->data['tasks'], 'taxonomies', array());
    ...
    function process_users(){
        ...
        // Get task
        $user_roles = acfe_get($this->data['tasks'], 'users', array());
    ...
    function process_options_pages(){
        ...
        // Get task
        $options_pages = acfe_get($this->data['tasks'], 'options_pages', array());
    ...
    function update_meta($meta, $post_id, $confirm = false){
        ...
        foreach($meta as $row){
            ...
            if($confirm){
                ...
                $save_as_meta = acfe_get($field, 'acfe_save_meta');
    ...
    function count_posts($post_type){
        ...
        $query = new WP_Query(array(
            'post_type'         => acfe_as_array($post_type),
            'post_status'       => 'any',
            'posts_per_page'    => 1,
        ));
    ...
    function count_terms($taxonomy){
        ...
        return wp_count_terms(array(
            'taxonomy'  => acfe_as_array($taxonomy),
            'number'    => 1,
            'hide_empty'=> false,
        ));
    ...
    function count_users($user_role){
        ...
        foreach($count['avail_roles'] as $role => $c){
            ...
            if(!in_array($role, acfe_as_array($user_role))) continue;
