...
class acfe_field_date_range_picker extends acfe_field{
    ...
    function render_field_settings($field){
        ...
        // return type
        acf_render_field_setting($field, array(
            'label'         => __('Return Type','acfe'),
            'instructions'  => __('The type of value to return in template functions','acf'),
            'type'          => 'radio',
            'name'          => 'return_type',
            'choices'       => array(
                'array'  => 'Array',
    ...
    function render_field($field){
        ...
        if(acfe_get($value, 'start') && acfe_get($value, 'end')){
    ...
    function update_value($value, $post_id, $field){
        ...
        if(!is_array($value)){
            ...
            $value = array(
                'start' => acfe_get($values, 0, ''), // first
                'end'   => acfe_get($values, 1, ''), // second
            );
    ...
    function load_any_value($value, $post_id, $field){
        ...
        if($this->is_sub_field($field)){
            ...
            if($sub_name && is_array($value)){
                return acfe_get($value, $sub_name);
            }
    ...
    function format_front_value($formatted, $unformatted, $post_id, $field, $form){
        ...
        if(!empty($formatted) && is_array($formatted)){
            ...
            $start = acfe_get($formatted, 'start');
            if(!empty($start)){
                $array[] = $start;
            }

            $end = acfe_get($formatted, 'end');
            if(!empty($end)){
                $array[] = $end;
            }
