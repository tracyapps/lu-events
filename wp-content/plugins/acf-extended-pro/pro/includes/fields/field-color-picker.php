...
class acfe_field_color_picker extends acfe_field_extend{
    ...
    function initialize(){
        ...
        // ACF 6.6+
        add_filter('acf/prepare_field/name=show_custom_palette', array($this, 'prepare_show_custom_palette'), 5);
        add_filter('acf/prepare_field/name=show_color_wheel',    array($this, 'prepare_show_color_wheel'), 5);
        add_filter('acf/validate_field/type=color_picker',       array($this, '_validate_field'), 5);
    ...
    /**
     * prepare_show_custom_palette
     *
     * @param $field
     *
     * @return mixed
     */
    function prepare_show_custom_palette($field){
    ...
    /**
     * prepare_show_color_wheel
     *
     * @param $field
     *
     * @return mixed
     */
    function prepare_show_color_wheel($field){
    ...
    function _validate_field($field){
        ...
        // ACF 6.6+: check if the field was saved with ACFE
        if(isset($field['display']) && !isset($field['acfe_advanced'])){
            $field['acfe_advanced'] = true;
        }
    ...
    function validate_field($field){
        ...
        if(!$this->is_old_acf){
            ...
            // old alpha
            if(acfe_get($field, 'alpha')){
                $field['enable_opacity'] = true;
                unset($field['alpha']);
            }
            
            // old return format
            if(acfe_get($field, 'return_format') === 'value'){
    ...
    function render_field_settings($field){
        ...
        // prepare advanced settings instructions
        $instructions = __('Show advanced settings', 'acfe');
        if(acfe_is_acf('6.6')){
            $instructions = __('Render Color Picker with ACF Extended settings', 'acfe');
        }
        
        acf_render_field_setting($field, array(
            'label'         => __('Advanced Color Picker', 'acfe'),
            'name'          => 'acfe_advanced',
            'key'           => 'acfe_advanced',
        ...
        acf_render_field_setting($field, array(
            ...
            'instructions'  => __('Choose how the color picker should be displayed', 'acfe'),
            'name'          => 'display',
            'type'          => 'select',
            'choices'       => array(
                'default'   => __('Default', 'acfe'),
                'palette'   => __('Palette', 'acfe'),
            ),
        ...
        acf_render_field_setting($field, array(
            ...
            'instructions'  => __('Choose the select button label', 'acfe'),
            'name'          => 'button_label',
            'type'          => 'text',
        ...
        acf_render_field_setting($field, array(
            ...
            'instructions'  => __('Display the color wheel', 'acfe'),
            'name'          => 'color_picker',
            'type'          => 'true_false',
            'ui'            => 1,
            'conditional_logic' => array(
        ...
        acf_render_field_setting($field, array(
            ...
            'instructions'  => __('Open the color wheel over other page elements', 'acfe'),
            'name'          => 'absolute',
            'type'          => 'true_false',
            'ui'            => 1,
            'conditional_logic' => array(
        ...
        acf_render_field_setting($field, array(
            ...
            'instructions'  => __('Show the text input allowing to enter a custom color', 'acfe'),
            'name'          => 'input',
            'type'          => 'true_false',
            'ui'            => 1,
            'conditional_logic' => array(
        ...
        acf_render_field_setting($field, array(
            ...
            'instructions'  => __('Allow users to deselect a color', 'acfe'),
            'name'          => 'allow_null',
            'type'          => 'true_false',
            'ui'            => 1,
            'conditional_logic' => array(
        ...
        if($this->is_old_acf){
            ...
            acf_render_field_setting($field, array(
                'label'         => __('RGBA','acf'),
                'instructions'  => __('Enable RGBA support', 'acfe'),
                'name'          => 'alpha',
                'type'          => 'true_false',
                'ui'            => 1,
            ));
        ...
        acf_render_field_setting($field, array(
            ...
            'instructions'  => __('Automatically add colors defined in <code>theme.json</code> and theme <code>editor-color-palette</code>', 'acfe'),
            'name'          => 'theme_colors',
            'type'          => 'true_false',
            'ui'            => 1,
            'conditional_logic' => array(
        ...
        acf_render_field_setting($field, array(
            ...
            'instructions'  => __('Add custom colors. Enter each choice on a new line. Example:', 'acf') . '<br /><br />' . "#2271b1 : Primary<br/>#6c757d : Secondary",
            'type'          => 'textarea',
            'name'          => 'colors',
            'conditional_logic' => array(
                array(
    ...
    function field_wrapper_attributes($wrapper, $field){
        ...
        if(!empty($field['acfe_advanced'])){
            $wrapper['data-acfe-advanced'] = true;
        }
    ...
    function render_field($field){
        ...
        // render native ACF Color Picker
        if(empty($field['acfe_advanced'])){
            return $this->instance->render_field($field);
        }
    ...
    function get_color_objects($field){
        ...
        if($field['theme_colors']){
            ...
            $editor_color_palette = current(acfe_as_array(get_theme_support('editor-color-palette')));
            ...
            if(!empty($editor_color_palette)){
                ...
                foreach($editor_color_palette as $row){
                    $colors[ $row['color'] ] = array(
                        'color' => $row['color'],
                        'slug'  => acfe_get($row, 'slug', sanitize_title($row['name'])),
                        'label' => $row['name'],
                    );
                }
            ...
            $editor_gradient_presets = current(acfe_as_array(get_theme_support('editor-gradient-presets')));
