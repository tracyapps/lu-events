        ...
        onValidationSuccess: function($form, validator) {
            ...
            if (this.get('screen') === 'post') {
                ...
                if ($reviewSync.length) {
                    ...
                    // todo: localize text
                    if (!confirm(acf.__('Local file is different from the version in database.') + "\n" + acf.__('Do you want to replace the local file with the current settings?'))) {
...
(function($) {
     ...
     * Admin Screen
     *
     * Append current page name (ie: post-new.php) to the "acf/ajax/check_screen" ajax request
     * Useful for custom locations that use $pagenow to match the screen (ie: Post Screen location)
     */
    new acf.Model({
        filters: {
            'check_screen_args': 'checkScreenArgs',
        },
        checkScreenArgs: function(ajaxData) {

            if (!ajaxData.pagenow) {
                ajaxData.pagenow = acfe.getCurrentFilename();
            }
...
(function($) {
    ...
    /**
     * ACF Select2 Woocommerce v2 - advanced fix
     *
     * This makes selection displayed as plain text with removed HTML tags
     * But results are still able to display HTML tags
     *
     * Note: This hook remove the execution of the hook 'select2_escape_markup' used in v1
     */
    acf.addFilter('select2_args', function(args) {
        ...
        // this is only used for selection (not results)
        args.escapeMarkup = function(markup) {
            // check if markup is a jQuery element and return the plain text
            return typeof markup.text === 'function' ? markup.text() : markup;
        }

        args.templateSelection = function(selection, $container) {

            $container = $($container);
            $container.removeAttr('data-value');

            if (selection.id) {
                $container.attr('data-value', selection.id);
            }

            var text = acf.escHtml(selection.text);
