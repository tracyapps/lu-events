...
class acfe_module_template_features{
    ...
    function rule_types($choices){
        ...
        $name = __('Forms', 'acf');
        $choices[ $name ] = acfe_after($choices[ $name ], 'options_page', array('acfe_template' => __('Template', 'acfe')));
    ...
    function rule_match_template($match, $rule, $screen, $field_group){
        ...
        if(acfe_get($screen, 'post_type') !== $this->module->post_type){
            return $match;
        }
        
        // get post id
        $post_id = acfe_get($screen, 'post_id', 0);
     ...
     * pre_load_reference
     *
     * Used by get_field() to retrieve the field key > field array
     *
     * @param $null
     * @param $field_name
     * @param $post_id
    ...
    function pre_load_reference($null, $field_name, $post_id){
        ...
        // validate post id
        if(!$this->validate_template_id($post_id)){
            return $null;
        }
        
        // get field from template
        $field = $this->get_template_field($field_name, $post_id);
        
        // found field, return key
        if(!empty($field)){
            return $field['key'];
        }
    ...
    function pre_load_value($null, $post_id, $field){
        ...
        // get template values
        $values = $this->get_template_values($post_id);
        
        if(!empty($values)){
            
            // loop values
            foreach($values as $value_name => $value_value){
    ...
    function pre_load_meta($null, $post_id){
        ...
        $fields = $this->get_template_fields($post_id);
        ...
        foreach($fields as $field){
            ...
            if(isset($values[ $field['name'] ])){
                ...
                // construct array
                $return[ "_$field_name" ] = $field['key'];
                $return[ $field_name ]    = $values[ $field['name'] ];
     ...
     * @return array|null
     */
    function get_template_values($post_id){
        ...
        $template = acfe_get_module('template')->get_item($post_id);
        ...
        // get template values
        $values = acfe_get($template, 'values');
        $values = acfe_as_array($values);
     ...
     * @return array
     */
    function get_template_fields($post_id){
        ...
        // get template field groups
        $field_groups = $this->get_template_field_groups($post_id);
     ...
     * get_template_field
     *
     * Helper to retrieve the field from field groups assigned to a template
     *
     * @param $field_name
     * @param $post_id
     *
     * @return false|mixed
     */
    function get_template_field($field_name, $post_id){
     ...
     * @return array
     */
    function get_template_field_groups($post_id){
        ...
        // get template field groups
        return acf_get_field_groups(array(
            'post_id'   => $template['ID'],
     ...
     * @return bool
     */
    function validate_template_id($post_id){
        return is_numeric($post_id) && get_post_type($post_id) === $this->module->post_type;
    }
