...
(function($) {
    ...
    // vars
    var loadingMarked = false;

    acfe.withMarkedAPI = function(callback) {
        ...
        // marked will need to be loaded. Hook callback to action.
        acf.addAction('marked_api_loaded', callback);
        ...
        if (url) {
            ...
            $.ajax({
                ...
                cache: true,
                success: function() {
                    acf.doAction('marked_api_loaded');
                }
            });
        }
    ...
    // vars
    var loadingPurify = false;

    acfe.withPurifyAPI = function(callback) {
        ...
        // DOMPurify will need to be loaded. Hook callback to action.
        acf.addAction('purify_api_loaded', callback);
        ...
        if (url) {
            ...
            $.ajax({
                ...
                cache: true,
                success: function() {
                    acf.doAction('purify_api_loaded');
                }
            });
        }
...
(function($) {
    ...
    new acf.Model({
        actions: {
            'acfe/new_ai_chat_message': 'newMessage'
        },
        newMessage: function(message) {

            // only will with ai
            if (!message.field.get('fillWithAi')) {
                return;
            }
    ...
    // global index for field instances
    // used for local storage keys when history is enabled
    var aiChatKeys = {};

    /**
     * Field: AI Chat
     */
    var AIPrompt = acf.Field.extend({
        ...
        actions: {
            'ready': 'scrollLogsBottom'
        },

        events: {
            'click .acfe-ai-prompt-button': 'onClickButton',
            'click .acfe-ai-prompt-clear>a': 'onClickClear',
            'input textarea': 'onInputTextarea',
            'keydown textarea': 'onKeydownTextarea',
        ...
        $promptInputWrap: function() {
            return this.$('.acfe-ai-prompt-input-wrap');
        },

        $chat: function() {
            return this.$('.acfe-ai-prompt-chat');
        },
        $button: function() {
            return this.$('.acfe-ai-prompt-button');
        },
        $clear: function() {
            return this.$('.acfe-ai-prompt-clear');
        },

        addMessage: function(args) {

            // instantiate
            var newMessage = acfe.newAIChatMessage(this, args);

            // append to messages
            var messages = this.getMessages().concat(newMessage);
            this.set('messages', messages);

            // append to logs
            this.$chat().append(newMessage.$el);
        ...
        initialize: function() {
            acfe.withMarkedAPI(this.initializeAPI.bind(this));
        },

        initializeAPI: function() {
            acfe.withPurifyAPI(this.render.bind(this));
        },

        render: function() {

            // use global keys index
            var index = aiChatKeys[this.get('key')] || 0;
            this.set('i', index);
            aiChatKeys[this.get('key')] = index + 1;

            // initialize history
            this.initializeHistory();

            // resize textarea (in case of default prompt)
            this.onInputTextarea(null, this.$input());

            if (this.get('display') === 'modal') {
        ...
        initializeHistory: function() {
            ...
            // get preferences
            var messages = this.getStoreData();
            if (!messages.length) {
                return;
            }
            ...
            // scroll to last message
            // use setTimeout when added as repeater row
            this.setTimeout(this.scrollLogsBottom);
        ...
        onInputTextarea: function(e, $el) {
            $el.height(0).height($el[0].scrollHeight); // resize textarea based on content
        },

        onKeydownTextarea: function(e, $el) {

            // handle enter key
            if (!e.shiftKey && e.key === 'Enter') {
                e.preventDefault();
                this.$button().first().trigger('click'); // trigger button click

                // if user press arrow up and the cursor is at the beginning of the textarea, populate it with the last user message
            } else if (e.key === 'ArrowUp' && $el[0].selectionStart === 0) {
                var lastUserMessage = this.getLastMessage('user');
                if (lastUserMessage) {
                    $el.val(lastUserMessage.get('text'));
                    this.onInputTextarea(e, $el); // trigger input event to resize textarea
                }


            }

            // do the same for arrow down but with the next user message and only if the cursor is at the end of the textarea
            else if (e.key === 'ArrowDown' && $el[0].selectionStart === $el.val().length) {
                var messages = this.getMessages();
                ...
                if (lastUserMessageIndex !== -1) {
                    var nextUserMessage = null;
                    for (var j = lastUserMessageIndex + 1; j < messages.length; j++) {
                        if (messages[j].get('role') === 'user') {
                            nextUserMessage = messages[j];
                            break;
                        }
                    }
                    if (nextUserMessage) {
                        $el.val(nextUserMessage.get('text'));
                        this.onInputTextarea(e, $el); // trigger input event to resize textarea
                    } else {
                        $el.val('');
                        this.onInputTextarea(e, $el); // trigger input event to resize textarea
        ...
        onChangeTextarea: function(e, $el) {
            e.stopPropagation(); // prevent exit message when refreshing
        },

        onFocusTextarea: function(e, $el) {
            this.$promptInputWrap().addClass('-focus');
        },

        onBlurTextarea: function(e, $el) {
            this.$promptInputWrap().removeClass('-focus');
        },
        ...
        onClickButton: function(e, $el) {
            ...
            if (mode !== 'text' && mode !== 'image') {
                ...
                return this.addMessage({
                    role: 'model',
                    text: acf.__('An error has occurred. Please try again.'),
                    reveal: false,
                    save: false,
                });
            ...
            // prepare args
            var args = {
                action: 'acfe/fields/ai_chat/query',
            ...
            // history
            // remove any message with mode === 'image' to avoid sending large base64 data to the server
            var history = this.getMessages().filter(function(message) {
                return message.get('mode') !== 'image';
        ...
        ajaxQuery: function(args) {

            // filter args
            data = acf.applyFilters('acfe/fields/ai_chat/args', args, this);
            data = acf.applyFilters('acfe/fields/ai_chat/args/name=' + this.get('name'), args, this);
            data = acf.applyFilters('acfe/fields/ai_chat/args/key=' + this.get('key'), args, this);

            // ajax
            $.ajax({
                url: acf.get('ajaxurl'),
                data: acf.prepareForAjax(args),
                type: 'post',
                dataType: 'json',
                context: this,
                beforeSend: function() {
                    this.ajaxBeforeSend(args);
                },
        ...
        ajaxSuccess: function(response) {
            ...
            var message = this.getLastMessage('model');
            ...
            if (!response || !response.success) {
                ...
                var text = response && response.message ? response.message : acf.__('An error has occurred. Please try again.');
        ...
        getStore: function() {
            ...
            // vars
            var data = acf.getPreference('this.acfe-ai-chat') || {};
            var key = this.get('key');
            var i = this.get('i');

            // init key + index bucket in one expression
            data[key] = data[key] || [];
        ...
        setStore: function(data) {
            acf.setPreference('this.acfe-ai-chat', data);
        },

        getStoreData: function() {
            var store = this.getStore();
            return store.data[store.key][store.i];
        },

        appendStore: function(obj) {
            var store = this.getStore();
            store.data[store.key][store.i].push(obj);
            this.setStore(store.data);
        },

        clearStore: function() {
            var store = this.getStore();
            store.data[store.key][store.i] = [];
            this.setStore(store.data);
        },

    });

    acf.registerFieldType(AIPrompt);
...
(function($) {
    ...
    /**
     * acfe.newAIChatMessage
     *
     * @param field
     * @param args
     * @returns {*}
     */
    acfe.newAIChatMessage = function(field, args) {

        var message = new AIChatMessage(field, args);

        acf.doAction('acfe/new_ai_chat_message', message);
        return message;

    }

    /**
     * Field: AI Chat - Message
     */
    var AIChatMessage = acf.Model.extend({
        scope: '.message',

        events: {
            'revealStep': 'scheduleScroll',
            'revealDone': 'scroll',
            'click [data-event="apply"]': 'onClickApply',
            'click [data-event="append"]': 'onClickAppend',
            'click [data-event="copy"]': 'onClickCopy',
        },
        ...
        setup: function(field, args) {

            // field
            this.field = field;

            // default args
            args = acf.parseArgs(args, this.data);
            if (!args.date) {
        ...
        update: function(args) {
            this.data = acf.parseArgs(args, this.data);
            this.render();
        },
        ...
        date: function(date) {
            this.$('.date').html(this.getTime(date)).attr('title', date);
        },
        ...
        actions: function(actions) {
            ...
            actions = acfe.getArray(actions);
            actions.map(function(action) {
                var $action = $(`<a href="#" data-event="${action.event}">${action.label}</a>`);
        ...
        reset: function() {
            ...
            if (this.get('role') === 'user') {
                this.addClass('question').removeClass('response');
            } else if (this.get('role') === 'model') {
                this.addClass('response').removeClass('question');
            }
        ...
        getAuthor: function() {
            ...
            if (this.get('role') === 'user') {
                ...
                if (this.get('provider')) {
                    author = this.get('provider');
                }
                if (this.get('model')) {
                    author += (author ? ' (' : '') + this.get('model') + (author ? ')' : '');
                }
            }
        ...
        getTime: function(isoDate) {
            ...
            // convert "2026-05-18T04:58:36.406Z" to H:i:s format
        ...
        maybePurify: function(text) {
            return this.hasPurify() ? DOMPurify.sanitize(marked.parse(text)) : text;
        },
        hasPurify: function() {
        ...
        save: function() {
            if (this.get('save')) {
                this.field.appendStore(this.get());
            }
        },
        setContent: function(text, count, speed) {
            ...
            if (!this.get('reveal')) {
                ...
                // try to purify
                text = this.maybePurify(text);

                this.hasPurify() ? $content.html(text) : $content.text(text);
            ...
            // cancel previous reveal on same element
            if (el._streamRevealTimer) {
                clearInterval(el._streamRevealTimer);
            }

            // reset content
            $content.html('');

            timer = setInterval(this.proxy(function() {

                // split into chunks
                index = Math.min(index + count, total);
                var chunk = words.slice(0, index).join(' ');

                // try to purify
                chunk = this.maybePurify(chunk);

                // render chunk
                this.hasPurify() ? $content.html(chunk) : $content.text(chunk);

                // notify each render step (live scroll hook)
                this.trigger('revealStep', [index, total, chunk]);

                // done
                if (index >= total) {
                    clearInterval(timer);
                    el._streamRevealTimer = null;
            ...
            el._streamRevealTimer = timer;
        },

        scheduleScrollToMessage: function(options) {
            if (this._scrollRaf) {
                return;
            }
            this._scrollRaf = requestAnimationFrame(this.proxy(function() {
                this._scrollRaf = null;
                this.scrollToMessage(options);
            }));
        },
        ...
        scrollToMessage: function(options) {
            ...
            // defaults
            options = options || {};
            var smooth = options.smooth !== false; // default true
            var alignEnd = options.alignEnd !== false; // default true (scroll to bottom of response)
            var offset = options.offset || 12; // extra px padding
            ...
            var response = this.$el[0];

            // position response relative to container scroll context
            // target = response top - container top + current scrollTop
            var responseTop = response.getBoundingClientRect().top;
            var containerTop = container.getBoundingClientRect().top;
            var targetTop = (responseTop - containerTop) + container.scrollTop;

            // optionally align to response bottom
            if (alignEnd) {
                targetTop = targetTop + response.offsetHeight - container.clientHeight + offset;
            } else {
        ...
        onClickApply: function(e, $el) {
            ...
            // assign value
            parentField.val(this.get('text'));

            // modal
            var modal = this.field.getModal();
            if (modal && modal.open) {
                modal.close();
            }

            // hook
            acf.doAction('acfe/fields/ai_fill/apply', this.get('text'), parentField, this);

        },

        onClickAppend: function(e, $el) {
            ...
            // hook
            acf.doAction('acfe/fields/ai_fill/append', this.get('text'), parentField, this);
        ...
        onClickCopy: function(e, $el) {
            ...
            // copy clipboard
            acfe.copyClipboard(this.get('text'), null, function(args) {
            ...
            // get parent field
            var parentField = this.field.parent() || null;

            // hook
            acf.doAction('acfe/fields/ai_fill/copy', this.get('text'), parentField, this);
...
(function($) {
    ...
    var Block_Editor = acf.Field.extend({
        ...
        initialize: function() {
            ...
            if (typeof wp.attachEditor !== 'undefined') {
...
(function($) {
    ...
    var PhoneNumber = acf.Field.extend({
        ...
        initialize: function() {
            ...
            // classic editor init
            // or: gutenberg init (this is required to init phone number in repeater on gutenberg)
            if (acf.get('editor') === 'classic' || (acf.get('editor') === 'block' && acf.didAction('load'))) {
        ...
        render: function() {
            ...
            // iti initialize
            iti.promise.then(this.proxy(function() {
                this.itiInitialize();
            }));

            // input open dropdown
            input.addEventListener('open:countrydropdown', this.proxy(function() {
                this.resizeDropdown();
            }));
        ...
        itiInitialize: function() {
            ...
            // actions
            acf.doAction(`acfe/fields/phone_number/init`, this);
            acf.doAction(`acfe/fields/phone_number/init/name=${this.get('name')}`, this);
            acf.doAction(`acfe/fields/phone_number/init/key=${this.get('key')}`, this);
        ...
        prepareArgs: function() {
            ...
            // utils library (CDN)
            args.utilsScript = 'https://cdnjs.cloudflare.com/ajax/libs/intl-tel-input/18.3.0/js/utils.min.js';

            // utils library (local)
            var int_tel_input_utils = acf.get('int_tel_input_utils');
            if (int_tel_input_utils) {
                args.utilsScript = int_tel_input_utils;
            }
            ...
            if (this.get('geolocation')) {
                ...
                args.geoIpLookup = function(success, failure) {
                    ...
                    // ajax data
                    var ajaxData = {
                        action: 'acfe/fields/phone_number/geolocation',
                        field_key: self.get('key'),
                    ...
                    $.ajax({
                        ...
                        // use first allowed country code or preferred country code
                        if (onlyCountries.length && !acfe.inArray(countryCode, onlyCountries)) {
                            countryCode = preferredCountries.length ? preferredCountries[0] : onlyCountries[0];
                        }
            ...
            // filters
            args = acf.applyFilters(`acfe/fields/phone_number/args`, args, this);
            args = acf.applyFilters(`acfe/fields/phone_number/args/name=${this.get('name')}`, args, this);
            args = acf.applyFilters(`acfe/fields/phone_number/args/key=${this.get('key')}`, args, this);
